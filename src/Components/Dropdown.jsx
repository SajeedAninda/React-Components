...
const Dropdown = () => {
    ...
    return (
        <div className='h-screen bg-gray-100 flex pt-14 flex-col items-center'>
            <img onClick={()=>{setOpen(!open)}} className='w-[100px] rounded-full h-[100px] cursor-pointer hover:shadow-2xl' src={avatar} alt="" />

            {
                open && <div>
                    <ul className='p-8 bg-white mt-2 rounded-xl shadow-xl'>
                        {menus.map(menu => (
